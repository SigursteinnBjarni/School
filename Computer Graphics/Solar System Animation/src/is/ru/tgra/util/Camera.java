...
public class Camera {
	...
	public void setModelViewMatrix() {
		...
		matrix[4] = u.y;
		matrix[8] = u.z;
		matrix[12] = Vector3D.dot(minusEye, u);
		matrix[1] = v.x;
		matrix[5] = v.y;
		matrix[9] = v.z;
		matrix[13] = Vector3D.dot(minusEye, v);
		matrix[2] = n.x;
		matrix[6] = n.y;
		matrix[10] = n.z;
		matrix[14] = Vector3D.dot(minusEye, n);
		matrix[3] = 0;
		matrix[7] = 0;
		matrix[11] = 0;
		matrix[15] = 1;
	...
	public void slide(float delU, float delV, float delN) {
		eye.add(Vector3D.sum(Vector3D.mult(delU, u),
				Vector3D.sum(Vector3D.mult(delV, v), Vector3D.mult(delN, n))));
		System.out.println("x: " + eye.x + " y: " + eye.y + " z: " + eye.z);
	}
